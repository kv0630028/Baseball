...
public class NestedScrollManager : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private float buttonAnimationTime = 0.5f;
    ...
    void Start()
    {
        ...
        buttonSize = (Screen.width / (BtnRect.Length + 1));
        Debug.Log($"버튼 사이즈 찍어보기 : {Screen.width} / {BtnRect.Length} / {buttonSize}" );
    }
    ...
    void Update()
    {
        ...
        if (!isDrag)
        {
            ...
            // 목표 버튼은 크기가 커짐
            for (int i = 0; i < SIZE; i++) BtnRect[i].sizeDelta = new Vector2(i == targetIndex ? buttonSize * 1 : buttonSize, BtnRect[i].sizeDelta.y);
        }
        ...
        for (int i = 0; i < SIZE; i++)
        {
            ...
            BtnImageRect[i].anchoredPosition3D = Vector3.Lerp(BtnImageRect[i].anchoredPosition3D, BtnTargetPos, buttonAnimationTime);
            BtnImageRect[i].localScale = Vector3.Lerp(BtnImageRect[i].localScale, BtnTargetScale, buttonAnimationTime);
